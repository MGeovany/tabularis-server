...
from app.services.conversion import convert_pdf_to_excel, ConversionError
from app.services.usage_limits import check_can_convert, increment_usage, UsageLimitExceeded
...
def pdf_to_excel(
    ...
    # Optional: X-Forwarded-For when behind proxy
    if "x-forwarded-for" in request.headers:
        ip = request.headers["x-forwarded-for"].split(",")[0].strip()

    log_audit(db, current_user.id, "CONVERSION_REQUEST", ip=ip, user_agent=user_agent)
    ...
    try:
        xlsx_bytes, duration_sec = convert_pdf_to_excel(
            content,
            filename,
        ...
        db.add(rec)
        db.commit()
        log_audit(db, current_user.id, "CONVERSION_FAILED", ip=ip, user_agent=user_agent)
    ...
    # Success: record conversion, increment usage, audit
    rec = Conversion(
        id=conversion_id,
        user_id=current_user.id,
        filename=filename,
    ...
    log_audit(db, current_user.id, "CONVERSION_SUCCESS", ip=ip, user_agent=user_agent)
